...
export default function Page() {
  return (
    <main style={{ maxWidth: 1000, margin: "0 auto", padding: "36px 28px 80px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 1, background: T.line, border: `1px solid ${T.line}`, borderRadius: 12, overflow: "hidden", marginBottom: 34 }}>
        {CAPAS.map((c) => (
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 12, overflow: "hidden", marginBottom: 34 }}>
        {SENALES.map(([nombre, como, tipo]) => (
          <div key={nombre} style={{ display: "grid", gridTemplateColumns: "1.1fr 1.6fr 120px", gap: 20, padding: "16px 22px", borderBottom: "1px solid #f0ece2", alignItems: "baseline" }}>
            <div style={{ fontSize: 14.5, fontWeight: 600 }}>{nombre}</div>
            <div style={{ fontSize: 13.5, lineHeight: 1.5, color: T.ink2 }}>{como}</div>
            <div style={{ fontFamily: T.mono, fontSize: 10.5, color: tipo === "REGLA" ? T.muted : T.ia, textAlign: "right" }}>{tipo}</div>
          </div>
        ))}
      </div>
